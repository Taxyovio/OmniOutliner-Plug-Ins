...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		if (
			!columns.byTitle("DEVONthink ID") ||
			columns.byTitle("DEVONthink ID").type !== Column.Type.Text
		) {
		...
		selection.items.forEach(function (item) {
			...
			try {
				text = Lib.textToMD(item.valueForColumn(outlineColumn));
			} catch (err) {
				text = "";
			}
			try {
				comment = Lib.textToMD(item.valueForColumn(noteColumn));
			} catch (err) {
				comment = "";
			}
			...
			if (text != "") {
				...
				urlStr =
					"x-devonthink://x-callback-url/createtext?title=" +
					title +
					"&text=" +
					text +
					"&location=" +
		...
		if (selection.items.length > 1) {
			...
			var alertMessage =
				"The process can be accerelated if DEVONthink and OmniOutliner are on the same screen.\nContinue with multitasking?";
