...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		if (
			!columns.byTitle("Deck") ||
			!columns.byTitle("Front") ||
			!columns.byTitle("Type")
		) {
			...
			inputForm.validate = function (formObject) {
				if (
					!formObject.values["profileInput"] ||
					!formObject.values["profileInput"].trim()
				) {
			...
			formPromise.then(function (formObject) {
				...
				try {
					var multitasking = formObject.values["multitaskingInput"];
				} catch (err) {
					multitasking = false;
				}
				...
				selection.items.forEach(function (item) {
					try {
						var front = item.topic;
					} catch (err) {
						var front = "";
					}
					try {
						var back = item.valueForColumn(
							columns.byTitle("Back")
						).string;
					} catch (err) {
						var back = "";
					}
					try {
						var deck = item.valueForColumn(
							columns.byTitle("Deck")
						).string;
					} catch (err) {
						var deck = "Default";
					...
					try {
						var tags = item.valueForColumn(
							columns.byTitle("Tags")
						).string;
					} catch (err) {
						var tags = "";
					}
					try {
						var reference = item.valueForColumn(
							columns.byTitle("Reference")
						).string;
					} catch (err) {
						var reference = "";
					}
					try {
						var extra = item.valueForColumn(
							columns.byTitle("Extra")
						).string;
					} catch (err) {
						var extra = "";
					}
					try {
						var reverse = item.valueForColumn(
							columns.byTitle("Reverse")
						).name;
					} catch (err) {
						var reverse = "";
					}
					...
					if (front != "") {
						...
						if (type === "Basic" || type === "") {
							urlStr =
								"anki://x-callback-url/addnote?profile=" +
								profile +
								"&type=" +
								type +
								"&deck=" +
				...
				if (multitasking === true) {
					...
					var alertMessage =
						"Anki and OmniOutliner must be on the same screen.\nContinue with multitasking?";
					var alert = new Alert(alertTitle, alertMessage);
					alert.addOption("Cancel");
					alert.addOption("Continue");
					var alertPromise = alert.show();
	...
	Document.makeNew(function (doc) {
		...
		baseItem.addChild(baseItem.beginning, function (item) {
			...
			item.setValueForColumn(
				outline.columns
					.byTitle("Type")
					.enumeration.memberNamed("Input"),
				outline.columns.byTitle("Type")
			);
			item.setValueForColumn("Academia", outline.columns.byTitle("Deck"));
			item.setValueForColumn(
				"This is the back text for a input card.",
				outline.columns.byTitle("Back")
			);
			item.setValueForColumn(
				"tag1 tag2",
				outline.columns.byTitle("Tags")
			);
			item.setValueForColumn(
				"Wikipedia",
				outline.columns.byTitle("Reference")
			);
			item.setValueForColumn(
				"Extra notes",
				outline.columns.byTitle("Extra")
			);
		});
		baseItem.addChild(baseItem.beginning, function (item) {
		...
		baseItem.addChild(baseItem.beginning, function (item) {
			...
			item.setValueForColumn(
				outline.columns
					.byTitle("Type")
					.enumeration.memberNamed("Basic"),
				outline.columns.byTitle("Type")
			);
			item.setValueForColumn("Academia", outline.columns.byTitle("Deck"));
			item.setValueForColumn(
				"This is the back text for a basic card.",
				outline.columns.byTitle("Back")
			);
			item.setValueForColumn(
				"tag1 tag2",
			...
			item.setValueForColumn(
				outline.columns
					.byTitle("Reverse")
					.enumeration.memberNamed("No"),
				outline.columns.byTitle("Reverse")
			);
			item.setValueForColumn(
				"Wikipedia",
				outline.columns.byTitle("Reference")
		...
		baseItem.addChild(baseItem.beginning, function (item) {
			item.topic =
				'This is a template for the "Add to Anki" Omni Automation action. This action requires using appropriate configuration for card types in Anki. Three custom card types are defined with custom fields: {Basic: Front, Back, Reference, Reverse, Extra}, {Cloze: Text, Reference, Extra}, {Input: Front, Back, Reference, Extra}.';
		});
		doc.save();
