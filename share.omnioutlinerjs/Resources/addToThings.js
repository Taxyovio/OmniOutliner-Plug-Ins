...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		try {
			...
			if (
				columns.byTitle("When") &&
				columns.byTitle("When").type === Column.Type.Date
			) {
			...
			if (
				columns.byTitle("Deadline") &&
				columns.byTitle("Deadline").type === Column.Type.Date
			) {
			...
			if (authColumn && authColumn.type === Column.Type.Text) {
				...
				for (var i = 0; i < rootItem.descendants.length; i++) {
					var textObj =
						rootItem.descendants[i].valueForColumn(authColumn);
			...
			if (defaultWhen === true) {
				inputForm.addField(reminderField(defaultReminder));
			}
			...
			inputForm.validate = function (formObject) {
				...
				if (formObject.values["projectInput"] === true) {
					if (keys.indexOf("checklistInput") !== -1) {
						...
						formObject.removeField(
							formObject.fields[keys.indexOf("checklistInput")]
						);
					}
				} else {
					if (keys.indexOf("checklistInput") === -1) {
						formObject.addField(
							checklistField(defaultChecklist),
							keys.indexOf("notesInput") + 1
						);
					}
				}
				...
				if (formObject.values["updateInput"] === false) {
					if (keys.indexOf("authInput") !== -1) {
						...
						formObject.removeField(
							formObject.fields[keys.indexOf("authInput")]
						);
					}
				} else {
					if (keys.indexOf("authInput") === -1) {
						formObject.addField(
							authField(defaultAuth),
							keys.indexOf("updateInput") + 1
						);
					}
				}
				...
				if (formObject.values["whenInput"] === true) {
					if (keys.indexOf("reminderInput") === -1) {
						formObject.addField(
							reminderField(defaultReminder),
							keys.indexOf("whenInput") + 1
						);
					}
				} else {
					if (keys.indexOf("reminderInput") !== -1) {
						...
						formObject.removeField(
							formObject.fields[keys.indexOf("reminderInput")]
						);
					}
				}

				if (
					formObject.values["authInput"] === "" &&
					formObject.values["updateInput"] === true
				) {
					throw new Error(
						"Authentication token is required for update operation."
					);
				}
				return null;
			};
			...
			formPromise.then(function (formObject) {
				...
				if (linkBack) {
					if (
						!columns.byTitle("Things ID") ||
						columns.byTitle("Things ID").type !== Column.Type.Text
					) {
				...
				items.forEach((item, index) => {
					console.log(
						"Things object is being created ",
						index + 1,
						" out of ",
						items.length
					);
				...
				*/
					...
					if (
						urlColumn &&
						item.valueForColumn(urlColumn) &&
						item.valueForColumn(urlColumn).string.trim().length !==
							0
					) {
					...
					var attributes = {};
					try {
						attributes.title = Lib.textToMD(
							item.valueForColumn(titleColumn)
						).trim();
					} catch (err) {
						attributes.title = "";
					}
					try {
						...
						attributes.notes =
							"omnioutliner:///open?row=" +
							item.identifier +
							"\n\n" +
							Lib.textToMD(
								item.valueForColumn(notesColumn)
							).trim();
					} catch (err) {
						attributes.notes =
							"omnioutliner:///open?row=" + item.identifier;
					}
						...
						if (filteredChildren.length <= 100) {
							filteredChildren.forEach((child) => {
								...
								checklistItem.attributes = {
									title:
										Lib.textToMD(
											child.valueForColumn(titleColumn)
										).trim() +
										" " +
										childLink,
								};
					...
					if (includesDeadline) {
						var deadlineColumn = columnByTitle(
							dateColumns,
							"Deadline"
						);
				...
				URL.fromString(urlStr).call((result) => {
					...
					if (linkBack) {
						...
						items.forEach((item, index) => {
							...
							var thingsColumn = columnByTitle(
								textColumns,
								"Things ID"
							);
