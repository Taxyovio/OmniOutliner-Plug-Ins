		...
		formPromise.then(function (formObject) {
			...
			editor
				...
				.forEach(function (node, index) {
					...
					var paragraphStringArray = paragraphArray.map(function (
						par
					) {
					...
					var paragraphStringArray = paragraphStringArray.filter(
						(el) => {
							return el;
						}
					);
					...
					if (paragraphArrayLength > 1) {
						...
						Timer.repeating(0.05, function (timer) {
							...
							if (counter === repeats) {
								if (duplicateColumnTitles) {
									duplicateColumnTitles.forEach(
										(title, i) => {
											columns[
												duplicateColumnIndices[i]
											].title = title;
										}
									);
								}
								...
								node.object.valueForColumn(
									selectedColumn
								).string = paragraphStringArray
									.slice()
									.reverse()[0];
								counter = counter + 1;
							} else {
								...
								if (
									paragraphStringArray.slice().reverse()[
										counter
									]
								) {
									newNode.object.valueForColumn(
										selectedColumn
									).string = paragraphStringArray
										.slice()
										.reverse()[counter];
						...
						alert = new Alert(
							"Error",
							"Not enough paragraphs are found."
						);
						alert.show();
					}
				});
