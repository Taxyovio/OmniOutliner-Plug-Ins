...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		var allField = new Form.Field.Checkbox("allInput", "All", false);
		...
		if (selectedItem.hasChildren) {
			...
			var leavesField = function (bool) {
				return new Form.Field.Checkbox("leavesInput", "Leaves", bool);
			};
		...
		if (selectedItem.level > 1) {
			...
			var parentField = function (bool) {
				return new Form.Field.Checkbox("parentInput", "Parent", bool);
			};
		...
		if (selectedItem.parent.children.length > 1) {
			...
			inputForm.addField(
				followingCollateralDescendantsField(
					defaultFollowingCollateralDescendants
				)
			);
			...
			inputForm.addField(
				followingSiblingsField(defaultFollowingSiblings)
			);
			...
			inputForm.addField(
				precedingCollateralDescendantsField(
					defaultPrecedingCollateralDescendants
				)
			);
			...
			inputForm.addField(
				precedingSiblingsField(defaultPrecedingSiblings)
			);
		}
		...
		inputForm.validate = function (formObject) {
			...
			var followingCollateralDescendantsChosen =
				formObject.values["followingCollateralDescendantsInput"];
			var precedingCollateralDescendantsChosen =
				formObject.values["precedingCollateralDescendantsInput"];

			var followingSiblingsChosen =
				formObject.values["followingSiblingsInput"];
			var precedingSiblingsChosen =
				formObject.values["precedingSiblingsInput"];

			if (allChosen) {
				if (keys.indexOf("precedingSiblingsInput") !== -1) {
					defaultPrecedingSiblings = precedingSiblingsChosen;
					precedingSiblingsInputIndex = keys.indexOf(
						"precedingSiblingsInput"
					);
					formObject.removeField(
						formObject.fields[precedingSiblingsInputIndex]
					);
				}

				if (keys.indexOf("followingSiblingsInput") !== -1) {
					defaultFollowingSiblings = followingSiblingsChosen;
					followingSiblingsInputIndex = keys.indexOf(
						"followingSiblingsInput"
					);
					formObject.removeField(
						formObject.fields[followingSiblingsInputIndex]
					);
				}

				if (
					keys.indexOf("precedingCollateralDescendantsInput") !== -1
				) {
					defaultPrecedingCollateralDescendants =
						precedingCollateralDescendantsChosen;
					precedingCollateralDescendantsInputIndex = keys.indexOf(
						"precedingCollateralDescendantsInput"
					);
					formObject.removeField(
						formObject.fields[
							precedingCollateralDescendantsInputIndex
						]
					);
				}

				if (
					keys.indexOf("followingCollateralDescendantsInput") !== -1
				) {
					defaultFollowingCollateralDescendants =
						followingCollateralDescendantsChosen;
					followingCollateralDescendantsInputIndex = keys.indexOf(
						"followingCollateralDescendantsInput"
					);
					formObject.removeField(
						formObject.fields[
							followingCollateralDescendantsInputIndex
						]
					);
				}
				...
				if (keys.indexOf("ancestorsInput") !== -1) {
					...
					formObject.removeField(
						formObject.fields[ancestorsInputIndex]
					);
				}
				...
				if (keys.indexOf("childrenInput") !== -1) {
					...
					formObject.removeField(
						formObject.fields[childrenInputIndex]
					);
				}
				...
				if (keys.indexOf("descendantsInput") !== -1) {
					...
					formObject.removeField(
						formObject.fields[descendantsInputIndex]
					);
				}
			} else {
				if (selectedItem.hasChildren) {
					if (keys.indexOf("descendantsInput") === -1) {
						formObject.addField(
							descendantsField(defaultDescendants),
							descendantsInputIndex
						);
					}
				}
				...
				if (selectedItem.level > 1) {
					if (keys.indexOf("ancestorsInput") === -1) {
						formObject.addField(
							ancestorsField(defaultAncestors),
							ancestorsInputIndex
						);
					}
				}
				...
				if (selectedItem.parent.children.length > 1) {
					if (
						keys.indexOf("followingCollateralDescendantsInput") ===
						-1
					) {
						formObject.addField(
							followingCollateralDescendantsField(
					...
					if (
						keys.indexOf("precedingCollateralDescendantsInput") ===
						-1
					) {
						formObject.addField(
							precedingCollateralDescendantsField(
				...
				if (descendantsChosen) {
					if (keys.indexOf("leavesInput") !== -1) {
						...
						leavesInputIndex = keys.indexOf("leavesInput");
						formObject.removeField(
							formObject.fields[leavesInputIndex]
						);
					}
				...
				if (ancestorsChosen) {
					if (keys.indexOf("parentInput") !== -1) {
						...
						parentInputIndex = keys.indexOf("parentInput");
						formObject.removeField(
							formObject.fields[parentInputIndex]
						);
					}
				} else if (ancestorsChosen === false) {
					if (keys.indexOf("parentInput") === -1) {
						formObject.addField(
							parentField(defaultParent),
							parentInputIndex
						);
					}
				}
