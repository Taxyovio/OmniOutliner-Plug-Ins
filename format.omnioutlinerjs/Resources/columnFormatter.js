	...
	var action = new PlugIn.Action(function (selection, sender) {
		...
		formPromise.then(function (formObject) {
			...
			if (selectedColumnType === Column.Type.Date) {
				...
				formatterFormPromise = formatterForm.show(
					formatterFormPrompt,
					"Continue"
				);
				...
				formatterForm.validate = function (formObject) {
					...
					if (formObject.values["formatterInput"] === true) {
						if (keys.indexOf("timeStyleInput") !== -1) {
							defaultTimeStyle =
								formObject.values["timeStyleInput"];
							formObject.removeField(
								formObject.fields[
									keys.indexOf("timeStyleInput")
								]
							);
						}
						if (keys.indexOf("dateStyleInput") !== -1) {
							defaultDateStyle =
								formObject.values["dateStyleInput"];
							formObject.removeField(
								formObject.fields[
									keys.indexOf("dateStyleInput")
								]
							);
						}
						...
						if (keys.indexOf("calendarInput") === -1) {
							formObject.addField(
								calendarField(defaultCalendar),
								1
							);
						}
						if (keys.indexOf("timeZoneInput") === -1) {
							formObject.addField(
								timeZoneField(defaultTimeZone),
								2
							);
						}
					} else {
						if (keys.indexOf("timeZoneInput") !== -1) {
							defaultTimeZone =
								formObject.values["timeZoneInput"];
							formObject.removeField(
								formObject.fields[keys.indexOf("timeZoneInput")]
							);
						}
						if (keys.indexOf("calendarInput") !== -1) {
							defaultCalendar =
								formObject.values["calendarInput"];
							formObject.removeField(
								formObject.fields[keys.indexOf("calendarInput")]
							);
						}

						if (keys.indexOf("dateStyleInput") === -1) {
							formObject.addField(
								dateStyleField(defaultDateStyle),
								1
							);
						}
						...
						if (keys.indexOf("timeStyleInput") === -1) {
							formObject.addField(
								timeStyleField(defaultTimeStyle),
								2
							);
						}
					}
				...
				formatterFormPromise.then(function (formObject) {
					...
					if (selectedFormatter === false) {
						...
						selectedColumn.formatter = Formatter.Date.withStyle(
							selectedDateStyle,
							selectedTimeStyle
						);
					} else {
						...
						if (selectedTimeZone !== ogTimeZone) {
							selectedColumn.formatter.timeZone = new TimeZone(
								selectedTimeZone
							);
						}
					}
			...
			if (selectedColumnType === Column.Type.Duration) {
				...
				var ogUseVerboseFormat =
					selectedColumn.formatter.useVerboseFormat;
				...
				formatterForm.validate = function (formObject) {
					if (
						isNaN(parseFloat(formObject.values["dayInput"])) ||
						isNaN(parseFloat(formObject.values["weekInput"]))
					) {
				...
				formatterFormPromise.then(function (formObject) {
					...
					var hoursPerWeek = parseFloat(
						formObject.values["weekInput"]
					);
			...
			if (selectedColumnType === Column.Type.Number) {
				...
				formatterForm.validate = function (formObject) {
					if (
						formObject.values["currencyInput"] !== "None" &&
						formObject.values["formatterInput"] !== "None"
					) {
						throw new Error(
							"Formatter and currency options are exclusive."
						);
					} else if (
						formObject.values["currencyInput"] === "None" &&
						formObject.values["formatterInput"] === "None"
				...
				formatterFormPromise.then(function (formObject) {
					...
					if (selectedFormatter !== "None") {
						selectedColumn.formatter = eval(
							"Formatter.Decimal." + selectedFormatter
						);
					} else if (selectedCurrency !== "None") {
						selectedColumn.formatter =
							Formatter.Decimal.currency(selectedCurrency);
					}
				});
