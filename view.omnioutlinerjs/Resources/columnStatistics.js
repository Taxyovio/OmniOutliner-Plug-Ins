...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		if (filteredColumns.length === 0) {
			throw new Error(
				"This document has no number nor duration columns."
			);
		}

		var filteredColumnTitles = filteredColumns.map(
			(column) => column.title
		);
		...
		formPromise.then(function (formObject) {
			...
			if (values.length % 2 === 0) {
				var median = values[values.length / 2 - 1]
					.add(values[values.length / 2])
					.divide(Decimal.one.add(Decimal.one));
			} else {
			...
			values.forEach((value) => {
				squaredSum = squaredSum.add(
					value.subtract(mean).multiply(value.subtract(mean))
				);
			});
			var squaredSumMean = squaredSum.divide(size);
			var standardDeviation = Decimal.fromString(
				Math.sqrt(parseFloat(squaredSumMean.toString())).toString()
			);
