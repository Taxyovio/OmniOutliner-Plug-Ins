...
(() => {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		var rgbField = function (str) {
			return new Form.Field.String("rgbInput", "RGB", str);
		};
		...
		inputForm.validate = function (formObject) {
			...
			if (keys.indexOf("rgbInput") === -1) {
				...
				if (formObject.values["colourInput"]) {
					...
					if (
						rgb.match(
							/\(\s*\d+\.{0,1}\d*\s*,\s*\d+\.{0,1}\d*\s*,\s*\d+\.{0,1}\d*\s*,\s*\d+\.{0,1}\d*\s*\)/
						) === null
					) {
						return false;
					} else {
					...
					formObject.removeField(
						formObject.fields[keys.indexOf("rgbInput")]
					);
				}
			}
			return textStatus;
