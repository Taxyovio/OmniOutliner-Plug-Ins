...
(() => {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		var defaultRule =
			"{%" + outlineColumn.title + "}-[{%Notes}]-({%Status})";
			...
			if (keys.indexOf("renamingRuleInput") === -1) {
				...
				if (formObject.values["automaticToggleInput"] === false) {
					...
					formObject.removeField(
						formObject.fields[keys.indexOf("renamingRuleInput")]
					);
				} else {
					if (
						formObject.values["renamingRuleInput"].match(
							/\{%.*?\}/g
						) === null
					) {
		...
		formPromise.then(function (formObject) {
			...
			if (automatic) {
				...
				matches = matches.filter((match) => {
					var columnTitle = match
						.replace(/^\{%/, "")
						.replace(/\}$/, "");
					return (
						match === "{%Status}" ||
						match === "{%Notes}" ||
						columns.byTitle(columnTitle)
					);
				});
	...
	selection.items.forEach((item, selectionIndex) => {
		textColumns.forEach((column) => {
			...
			if (textObj) {
				...
				var attachmentRanges = textObj.ranges(
					TextComponent.Attachments
				);
					...
					attachments.forEach((att, index) => {
						...
						if (wrapper.type === FileWrapper.Type.File) {
							...
							if (filename.length > displayNameLimit) {
								var displayName =
									baseName.substring(
										0,
										baseName.length -
											(filename.length - displayNameLimit)
									) +
									"...." +
									extension;
							} else {
							...
							if (matches && matches.length !== 0) {
								...
								var matchedStrings = matchedColumns.map(
									(col) => {
									...
									*/
										...
										if (col.type === Column.Type.Text) {
											...
											if (value) {
												charCount +=
													value.string.trim().length;
												return value.string.trim();
											} else {
												return "";
											}
										} else if (
											col.type === Column.Type.Checkbox
										) {
											...
											col.type === Column.Type.Enumeration
										) {
								...
								if (charCount < 100) {
									...
									var allowedCharCountPerMatch = Math.floor(
										99 / matches.length
									);
									matchedStrings.forEach((str, index) => {
										var subStr = str.substring(
											0,
											allowedCharCountPerMatch
										);
										var numberOfWords =
											subStr.match(/\b(\w+)\b/g).length;
										if (numberOfWords > 1) {
								...
								console.log(
									"base filename generated by renaming rule: ",
									baseName
								);
							}
							...
							inputForm.validate = function (formObject) {
								if (
									formObject.values["toggleInput"] === false
								) {
									return true;
								} else if (
									/\s/.test(
									...
									var textValue =
										formObject.values["baseNameInput"];
									var textStatus =
							...
							formPromise.then(function (formObject) {
								var skipRenaming =
									!formObject.values["toggleInput"];

								if (!skipRenaming) {
									var newBaseName =
										formObject.values["baseNameInput"];
									var newExtension =
										formObject.values["extensionInput"];
									if (legalise) {
										newBaseName =
											legaliseFilename(newBaseName);
									}
...
function rename(
				...
				if (attachments) {
					attachments.forEach((att, index) => {
						...
						if (wrapper.type === FileWrapper.Type.File) {
							...
							if (legalise) {
								newBaseName = legaliseFilename(newBaseName);
							}
							...
							if (wrapper.preferredFilename !== newFilename) {
								...
								var newTextObj = Text.makeFileAttachment(
									newWrapper,
									style
								);
								console.log(
	...
	if (numberOfAttachments === 0) {
		...
		var alertMessage =
			numberOfAttachments + " attachments have been renamed.";
	}
