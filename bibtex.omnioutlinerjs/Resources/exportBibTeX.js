...
var _ = (function () {
	var action = new PlugIn.Action(function (selection) {
		...
		var fileWrapper = FileWrapper.withContents(
			document.name + ".bib",
			data
		);
...
function exportBibTeX(items) {
	...
	if (!document.outline.columns.byTitle("EntryKey")) {
		...
		document.outline.columns.byTitle("EntryKey").type !== Column.Type.Text
	) {
	...
	if (!document.outline.columns.byTitle("EntryType")) {
		...
		document.outline.columns.byTitle("EntryType").type !== Column.Type.Text
	) {
	...
	items.forEach((item) => {
		var entryTypeText = item.valueForColumn(
			filteredColumns[filteredColumnTitles.indexOf("EntryType")]
		);
		var entryKeyText = item.valueForColumn(
			filteredColumns[filteredColumnTitles.indexOf("EntryKey")]
		);

		if (
			entryTypeText &&
			entryTypeText.string.trim().length !== 0 &&
			entryKeyText &&
			entryKeyText.string.trim().length !== 0
		) {
			// First line of a bibtex entry
			str +=
				"@" +
				entryTypeText.string.trim() +
				"{" +
				entryKeyText.string.trim() +
				",\n";
			...
			var fieldColumns = filteredColumns.filter(function (column) {
				if (
					column !==
						filteredColumns[
							filteredColumnTitles.indexOf("EntryType")
						] &&
					column !==
						filteredColumns[
							filteredColumnTitles.indexOf("EntryKey")
						]
				) {
					return column;
				}
			...
			fieldColumns.forEach((column) => {
				if (
					item.valueForColumn(column) &&
					item.valueForColumn(column).string.trim().length !== 0
				) {
					str +=
						"\t" +
						column.title +
						" = {" +
						item.valueForColumn(column).string.trim() +
						"},\n";
				}
			});
			str += "}\n\n";
