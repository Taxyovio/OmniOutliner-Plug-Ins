...
var _ = (function () {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		if (!document.outline.columns.byTitle("title")) {
			...
			document.outline.columns.byTitle("title").type !== Column.Type.Text
		) {
		...
		if (!document.outline.columns.byTitle("author")) {
			...
			document.outline.columns.byTitle("author").type !== Column.Type.Text
		) {
		...
		if (!document.outline.columns.byTitle("year")) {
			...
			document.outline.columns.byTitle("year").type !== Column.Type.Text
		) {
		...
		var titleColumns = document.outline.columns.filter(function (column) {
			if (column.type === Column.Type.Text && column.title === "title") {
				return column;
			}
		});
		...
		var authorColumns = document.outline.columns.filter(function (column) {
			if (column.type === Column.Type.Text && column.title === "author") {
				return column;
			}
		});
		...
		var yearColumns = document.outline.columns.filter(function (column) {
			if (column.type === Column.Type.Text && column.title === "year") {
				return column;
			}
		});
