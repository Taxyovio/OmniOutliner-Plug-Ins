...
(() => {
	var action = new PlugIn.Action(function (selection, sender) {
		...
		if (filteredColumnTitles.indexOf("attachment") !== -1) {
			...
			filteredColumns.indexOf(document.outline.outlineColumn) !== -1
		) {
		...
		inputForm.validate = function (formObject) {
			...
			if (formObject.values["importURLOptionInput"] === true) {
				if (keys.indexOf("importAppURLOptionInput") === -1) {
					formObject.addField(
						importAppURLOptionField(defaultimportAppURLOption)
					);
				}
			} else {
				if (keys.indexOf("importAppURLOptionInput") !== -1) {
					defaultimportAppURLOption =
						formObject.values["importAppURLOptionInput"];
					formObject.removeField(
						formObject.fields[
							keys.indexOf("importAppURLOptionInput")
						]
					);
				}
			}
		...
		formPromise.then(function (formObject) {
			...
			var importUnmatched =
				formObject.values["importUnmatchedOptionInput"];
...
function matchItem(
	...
	function autoMatch() {
		...
		if (repeats > words.length) {
			repeats = words.length;
		}
			...
			for (var i = 0; i < rootItem.descendants.length; i++) {
				...
				if (blacklist.indexOf(i) === -1) {
					...
					if (matchCount === words.length - pass) {
						...
						console.log(
							"Not a match this pass. But a potential match."
						);
					}
				} else {
	...
	if (app.platformName === "iOS" || app.platformName === "iPadOS") {
		...
		if (importAppURL) {
			...
			if (urlStr.includes(uuidGoodReader)) {
				...
				var str = matchedStr.substring(
					uuidGoodReader.length + "/Documents/".length,
					matchedStr.length
				);
