...
var _ = (function () {
	var action = new PlugIn.Action(function (selection) {
		...
		if (!document.outline.columns.byTitle("EntryKey")) {
			...
			document.outline.columns.byTitle("EntryKey").type !==
			Column.Type.Text
		) {
		...
		if (!document.outline.columns.byTitle("EntryType")) {
			...
			document.outline.columns.byTitle("EntryType").type !==
			Column.Type.Text
		) {
		...
		rootItem.descendants.forEach((item, index) => {
			var entryKeyText = item.valueForColumn(
				document.outline.columns.byTitle("EntryKey")
			);
			if (entryKeyText && entryKeyText.string.length !== 0) {
				...
				var entryTypeText = item.valueForColumn(
					document.outline.columns.byTitle("EntryType")
				);
				if (entryTypeText && entryTypeText.string.length !== 0) {
					if (entryKeyText) {
						entryKeyText.string =
							entryTypeText.string.trim() + index;
					} else {
						var text = new Text(
							entryTypeText.string.trim() + index,
							item.style
				...
				entryKeyText = item.valueForColumn(
					document.outline.columns.byTitle("EntryKey")
				);
				keysObj.keys.push(entryKeyText.string);
				keysObj.items.push(item);
		...
		if (hasDuplicates(keysObj.keys)) {
			var duplicateCount =
				keysObj.keys.length - keysObj.keys.filter(onlyUnique).length;
			...
			if (duplicateCount === 1) {
				...
				var alertMessage =
					"Rename " + duplicateCount + " duplicate entry keys?";
			}
			...
			alertPromise.then((buttonIndex) => {
				if (buttonIndex === 1) {
					...
					uniqueKeys.forEach((key, i) => {
						...
						var ogKeyText = item.valueForColumn(
							document.outline.columns.byTitle("EntryKey")
						);
