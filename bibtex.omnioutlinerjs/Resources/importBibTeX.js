...
var _ = (function () {
			...
			pickerPromise.then(function (urlsArray) {
				urlsArray.forEach((url) => {
					var filename = decodeURIComponent(
						url.string.substring(url.string.lastIndexOf("/") + 1)
					);
	...
	action.validate = function (selection) {
		if (document && selection.items.length <= 1) {
			return true;
		} else {
			return false;
		}
...
function importBibTeX(bibStr, Parser) {
	...
	entries.forEach((entry) => {
		if (columnTitles.indexOf("EntryKey") === -1) {
			...
			outline.addColumn(
				Column.Type.Text,
				editor.beforeColumn(outlineColumn),
				function (column) {
	...
	entries.forEach((entry) => {
		...
		item.addChild(position, function (item) {
			item.setValueForColumn(
				entry.EntryType,
				outline.columns.byTitle("EntryType")
			);
			item.setValueForColumn(
				entry.EntryKey,
				outline.columns.byTitle("EntryKey")
			);
			Object.getOwnPropertyNames(fields).forEach((fieldName) => {
				if (fields[fieldName] && fields[fieldName].trim()) {
					...
					item.setValueForColumn(
						textObj,
						columnByTitle(textColumns, fieldName.toLowerCase())
					);
				}
			});
		});
	...
	if (entries.length === 0) {
		if (bibJSON.errors.length === 0) {
			var alertMessage =
				"No bibtex entry has been imported, with no error.";
		} else if (bibJSON.errors.length === 1) {
			var alertMessage =
				"No bibtex entry has been imported, with 1 error.";
		} else {
			var alertMessage =
				"No bibtex entry has been imported, with " +
				bibJSON.errors.length +
				" errors.";
		}
	} else if (entries.length === 1) {
		if (bibJSON.errors.length === 0) {
			var alertMessage =
				"1 bibtex entry has been imported, with no error.";
		} else if (bibJSON.errors.length === 1) {
			var alertMessage =
				"1 bibtex entry has been imported, with 1 error.";
		} else {
			var alertMessage =
				"1 bibtex entry has been imported, with " +
				bibJSON.errors.length +
				" errors.";
		}
	} else {
		if (bibJSON.errors.length === 0) {
			var alertMessage =
				entries.length +
				" bibtex entries have been imported, with no error.";
		} else if (bibJSON.errors.length === 1) {
			var alertMessage =
				entries.length +
				" bibtex entries have been imported, with 1 error.";
		} else {
			var alertMessage =
				entries.length +
				" bibtex entries have been imported, with " +
				bibJSON.errors.length +
				" errors.";
		}
	}
