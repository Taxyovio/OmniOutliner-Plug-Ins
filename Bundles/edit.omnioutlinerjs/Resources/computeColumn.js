	...
	var action = new PlugIn.Action(function(selection, sender) {
		...
		filteredColumns = columns.filter(function(column) {
			if (editor.visibilityOfColumn(column)) {
				if (column.type === Column.Type.Number || column.type === Column.Type.Duration || column.type === Column.Type.Date || column.type === Column.Type.Checkbox) {
					return column
				}
			}
		})
		...
		if (filteredColumns.length === 0) {
			throw new Error("This document has no visible columns of valid types.")
		}
		...
		var inputColumnOptions = function(targetColumn, selectedInputColumns) {
			...
			var columns = filteredColumns.filter(function(column) {
				if (!selectedInputColumns.includes(column)) {return column}
			})
		...
		inputForm.validate = function(formObject) {
			...
			if (selectedTargetColumn && !targetIsFixed) {
				formObject.addField(columnField(inputColumnOptions(selectedTargetColumn, selectedInputColumns), 0, null))
				
				inputLength = inputLength + 1
				
				var index = keys.indexOf("targetInput")
				formObject.removeField(formObject.fields[index])
				formObject.addField(targetField([selectedTargetColumn], selectedTargetColumn), index)
				targetIsFixed = true
			}
			
			
			
			if (keys.indexOf("columnInput" + (inputLength - 1)) && formObject.values["columnInput" + (inputLength - 1)]) {
				
				var column = formObject.values["columnInput" + (inputLength - 1)]
				var index = keys.indexOf("columnInput" + (inputLength - 1))
				
				// Fix last column option
				if (selectedInputColumns.length === inputLength - 1) {
					formObject.removeField(formObject.fields[index])
					formObject.addField(columnField([column], inputLength - 1, column), index)
					...
					if (!formulaInput.match(regex)) {
						...
						formObject.removeField(formObject.fields[formulaFieldIndex])
						formObject.addField(formulaField(formulaInput), formulaFieldIndex)
					}
				}	
				if (!selectedInputColumns.includes(column)) {selectedInputColumns.push(column)}
				
				
				
				if (inputColumnOptions(selectedTargetColumn, selectedInputColumns).length !== 0 && keys.indexOf("columnInput" + inputLength) === -1) {
					
					
					
					// Add next column option
					formObject.addField(columnField(inputColumnOptions(selectedTargetColumn, selectedInputColumns), inputLength, null))
					
					inputLength = inputLength + 1
				}
				
				console.log(selectedInputColumns.length + " out of " + inputLength + " input column fields are selected on the form.")
				
			}
			
			// Two dates are only allowed to be substracted
			if (selectedInputColumns.length === 2 && selectedInputColumns[0].type === Column.Type.Date && selectedInputColumns[1].type === Column.Type.Date) {
			...
			if (formObject.values["targetInput"]) {
				if (isValidFormula(formulaInput, selectedInputColumns, formObject.values["targetInput"])) {
					return true
				} else {
...
function compute(formula, row, columns, target) {
	...
	if (result) {
		if ((target.type === Column.Type.Number || target.type === Column.Type.Duration) && typeof result !== "number") {
			result = null
		} else if (target.type === Column.Type.Checkbox && result.constructor.name !== "Sate") {
			result = State.Unchecked
